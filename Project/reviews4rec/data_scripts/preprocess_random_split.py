...
def get_map(data, key_user, key_item, k_core=0):
    ...
    while done < 500:
        ...
        prev_u = len(map_temp_u)
        prev_i = len(map_temp_i)
        done += 1
...
def load_beer(file):
    ...
    for line in tqdm(lines):
        ...
        if line[:11] == "beer/beerId":
            temp['asin'] = line.split(":")[-1]
        elif line[:18] == "review/profileName":
            temp['reviewerID'] = line.split(":")[-1][1:]
        elif line[:14] == "review/overall":
            temp['overall'] = float(line.split(":")[-1].split("/")[0])
        elif line[:11] == "review/text":
            temp['reviewText'] = line.split(":")[-1][1:]
...
for k_core in map(int, sys.argv[3].split(",")):
    ...
    for percent_to_keep in map(int, sys.argv[4].split(",")):
        ...
        for i in range(len(final[:train_split])):
            if random.random() > float(percent_to_keep) / 100.0:
                final[i][3] = []
            else:
                reviews_kept += 1.0
            total_reviews += 1.0
        ...
        # model = Word2Vec(all_reviews_train_word2vec, min_count=1, size=64, workers=-1, window=1, sg=1, negative=64,
        #                  iter=20).wv
        ...
        if percent_to_keep != 100:
            base_path += str(percent_to_keep) + '_percent/'
