...
def func(hyper_params, reader, name):
    ...
    with h5py.File(base_path + name + '.hdf5', 'w') as file:
        ...
        dset['b'] = file.create_dataset("b", [len(reader.data), 10], dtype='i8', maxshape=[len(reader.data), 10],
                                        compression="gzip")
        dset['c'] = file.create_dataset("c", [len(reader.data), 10], dtype='i8', maxshape=[len(reader.data), 10],
                                        compression="gzip")
        ...
        dset['f'] = file.create_dataset("f", [len(reader.data)], dtype='i8', maxshape=[len(reader.data)],
                                        compression="gzip")
        dset['g'] = file.create_dataset("g", [len(reader.data)], dtype='i8', maxshape=[len(reader.data)],
                                        compression="gzip")
        dset['h'] = file.create_dataset("h", [len(reader.data)], dtype='f8', maxshape=[len(reader.data)],
                                        compression="gzip")

        at = 0
        for (this_reviews, users_who_gave, items_reviewed, user_reviews, item_reviews, user,
             item), rating in reader.iter_review(simple=True):
