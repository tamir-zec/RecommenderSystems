...
def load_data_and_labels(train_data, valid_data, user_review, item_review, user_rid, item_rid):
    ...
    for line in f_train:
        ...
        if item_id in i_text:
            reid_item_train.append(i_rid[item_id])  #####write here
        else:
            i_text[item_id] = []
...
if __name__ == '__main__':
    ...
    for category in categories:
        ...
        tf.compat.v1.flags.DEFINE_string("valid_data", os.path.join(TPS_DIR, category + "_valid.csv"),
                                         "Data for validation")
        tf.compat.v1.flags.DEFINE_string("test_data", os.path.join(TPS_DIR, category + "_test.csv"), "Data for testing")
        tf.compat.v1.flags.DEFINE_string("train_data", os.path.join(TPS_DIR, category + "_train.csv"),
                                         "Data for training")
        tf.compat.v1.flags.DEFINE_string("user_review", os.path.join(TPS_DIR, category + "_user_review"),
                                         "User's reviews")
        tf.compat.v1.flags.DEFINE_string("item_review", os.path.join(TPS_DIR, category + "_item_review"),
                                         "Item's reviews")
        tf.compat.v1.flags.DEFINE_string("user_review_id", os.path.join(TPS_DIR, category + "_user_rid"),
                                         "user_review_id")
        tf.compat.v1.flags.DEFINE_string("item_review_id", os.path.join(TPS_DIR, category + "_item_rid"),
                                         "item_review_id")
        ...
        print('write begin')
        output = open(os.path.join(TPS_DIR, category +'.train'), 'wb')
        pickle.dump(batches_train, output)
        output = open(os.path.join(TPS_DIR, category + '.test'), 'wb')
        pickle.dump(batches_test, output)

        para = {}
        para['user_num'] = user_num
        para['item_num'] = item_num
        para['review_num_u'] = u_text[next(iter(u_text))].shape[0]
        para['review_num_i'] = i_text[next(iter(i_text))].shape[0]
        para['review_len_u'] = u_text[next(iter(u_text))].shape[1]
        para['review_len_i'] = i_text[next(iter(i_text))].shape[1]
        ...
        para['u_text'] = u_text
        para['i_text'] = i_text
        output = open(os.path.join(TPS_DIR, category + '.para'), 'wb')
