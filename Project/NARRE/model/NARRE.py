...
class NARRE(object):
    def __init__(
        ...
        self.input_reiid = tf.compat.v1.placeholder(tf.int32, [None, review_num_i], name='input_reuid')
        ...
        self.h_pool_u = pooled_outputs_u[0]
        ...
        self.h_pool_i = [pooled_outputs_i[0]]
        self.h_pool_flat_i = tf.reshape(self.h_pool_i, [-1, review_num_i, num_filters_total])

        with tf.name_scope("dropout"):
            self.h_drop_u = tf.compat.v1.nn.dropout(self.h_pool_flat_u, 1.0)
            self.h_drop_i = tf.compat.v1.nn.dropout(self.h_pool_flat_i, 1.0)
        ...
        with tf.name_scope("add_reviews"):
            self.u_feas = tf.compat.v1.reduce_sum(tf.multiply(self.u_a, self.h_drop_u), 1)
            self.u_feas = tf.compat.v1.nn.dropout(self.u_feas, self.dropout_rate)
            self.i_feas = tf.compat.v1.reduce_sum(tf.multiply(self.i_a, self.h_drop_i), 1)
            self.i_feas = tf.compat.v1.nn.dropout(self.i_feas, self.dropout_rate)
        ...
        with tf.name_scope('ncf'):
            ...
            self.FM = tf.nn.dropout(self.FM, self.dropout_rate)

            Wmul = tf.Variable(tf.random.uniform([n_latent, 1], -0.1, 0.1), name='wmul')

            self.mul = tf.matmul(self.FM, Wmul)
            self.score = tf.compat.v1.reduce_sum(self.mul, 1, keepdims=True)
        ...
        with tf.name_scope("accuracy"):
            self.mae = tf.compat.v1.reduce_mean(tf.abs(tf.subtract(self.predictions, self.input_y)))
            self.accuracy = tf.sqrt(tf.compat.v1.reduce_mean(tf.square(tf.subtract(self.predictions, self.input_y))))
